layui.use(['table', 'laytpl', 'laydate', 'form', 'common'], function() {
    ...
    var dataTable = common.renderTable({
	    ...
	    cols: [[
	      ...
	      {field:'refSignValue', title: '类目id', width: 300},
          {field:'ruleType', title: '规则类型', width: 200,templet:function(val,row){
            ...
            {field:'refSignName', title: '类目名称', width: 300},
	...
	$('.btn-add').click(function(data){
            layer.open({
                ...
                area: ['95%', '95%'],
                shadeClose: true,
                content: rootPath + "/coupons/activity/goods/rule/addcategory?ruleId="+ruleid+"&type="+rtype,
    	...
    	switch (obj.event) {
            case 'reed':
                ...
                layer.open({
                    ...
                    area: ['95%', '95%'],
                    shadeClose: true,
                    content: rootPath + "/coupons/activity/goods/rule/reedcatedetails?catetype="+rtype,
                    success:function (layero,index) {
                        $(".submit", layero.find("iframe")[0].contentWindow.document).hide();
                        $(".search", layero.find("iframe")[0].contentWindow.document).hide();
                        $("#id", layero.find("iframe")[0].contentWindow.document).val(data.refSignValue);
