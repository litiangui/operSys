layui.use(['table', 'laytpl', 'form', 'common'], function() {
	...
	form.on('submit(submit)', function(data) {
		...
		if(data.field.goodsCode==''){
			  layer.alert("请完善砍价商品信息",{icon: 2});
		      return false;
		}
		
	if(data.field.reductionGoodsPrice.indexOf("+")!=-1){
			  layer.alert("砍价价格不能有特殊字符(加号)",{icon: 2});
		      return false;
		  }
	 if(data.field.reductionNumRule.indexOf("+")!=-1){
		  layer.alert("砍价次数不能有特殊字符(加号)",{icon: 2});
	      return false;
	  }
	  if(data.field.reductionGoodsNum.indexOf("+")!=-1){
		  layer.alert("砍价商品数量不能有特殊字符(加号)",{icon: 2});
	      return false;
	  }
	...
	if(data.field.brokerageMoney.indexOf("+")!=-1){
		  layer.alert("佣金不能有特殊字符(加号)",{icon: 2});
	      return false;
	  }
	 ...
	 if(data.field.brokerageMoney==0){
		...
		common.wait("提交中...");
		common.post( rootPath + '/mongo/priceReductionGoods/save', data.field, function(msg) {
			common.hide();
			if (msg.ok) {
				// 提交成功刷新数据表格，关闭弹出层
    ...
    $(".search").click(function(){
        ...
        $.ajax({
            ...
            success: function (msg) {
            	...
            	$("#goodsCode").val(data.productCode);
