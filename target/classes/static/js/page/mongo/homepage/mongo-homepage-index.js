layui.use(['table', 'laytpl', 'form', 'common','laydate','element','upload'], function() {
	...
	form.render();
		...
		form.on('submit(submit)', function(data) {
			...
			common.wait("提交中...");
			common.post(rootPath + '/mongo/homepage/addBanner', data.field, function(msg) {
				common.hide();
				if (msg.ok) {
					// 提交成功刷新数据表格，关闭弹出层
	...
	// 初始化数据表格
		 var dataTable = common.renderTable({
			 height:500  //容器高度
            , url: common.getUrl('/mongo/homepage/bannerList')
    ...
    table.on('tool(data-list)', function(obj){
		...
		case 'delete':
            layer.confirm("确认要删除吗，删除后不能恢复", { title: "删除确认" }, function (index) {
                layer.close(index);
                common.post(rootPath + '/mongo/homepage/deleteBanner', {
