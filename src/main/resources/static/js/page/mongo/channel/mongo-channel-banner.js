layui.use(['table', 'laytpl', 'form', 'common','laydate','element','upload'], function() {
	...
	form.render();
		...
		$('.btn-add').click(function(){
            var uniqeKry= $("#bannerUniqeKey").val();
            if (uniqeKry===null||uniqeKry===''){
                layer.alert("请选择栏目");
                return false;
            }
            $('#imgDisplay').hide();
		...
		form.on('submit(submit)', function(data) {
			...
			common.wait("提交中...");
			common.post(rootPath + '/mongo/channel/addBanner', data.field, function(msg) {
				common.hide();
				if (msg.ok) {
					// 提交成功刷新数据表格，关闭弹出层
	...
	// 初始化数据表格
		 var dataTable = common.renderTable({
			 height:500  //容器高度
            , url: common.getUrl('/mongo/channel/bannerList')
            , where: $(".data-list-form").serializeJSON()
            , cols: [[
                {field: 'id', title: 'id', width: 180},
                {field: 'sortNum', title: '排序值', width: 180},
                {field: 'imgTitle', title: '图片标题', width: 180},
                {field: 'imgUrl', title: '图片',  width: 180,templet:function(val) {
                        return "<img style='width: 50px; height: 30px;'" +
                            "src='"+val.imgUrl+"'>"
                    }},
                {field: 'imgTarget', title: '图片跳转连接', width: 180},
    ...
    table.on('tool(data-list)', function(obj){
		...
		case 'delete':
            layer.confirm("确认要删除吗，删除后不能恢复", { title: "删除确认" }, function (index) {
                layer.close(index);
                common.post(rootPath + '/mongo/channel/deleteBanner', {
