...
import com.shq.oper.service.primarydb.BaseService;
...
public abstract class BaseServiceImpl<T, PK extends Serializable> implements BaseService<T, PK> {
    ...
    @Override
    @Transactional
    public int disabledById(T record){
    	int result = mapper.disabledById(record);
    	Assert.isTrue(result == 1, "禁用失败,禁用成功数量为["+result+"]!");//只用当 表达式为True,才通过。
    	return 1;
    ...
    @Override
    @Transactional
    public int enableById(T record){
    	int result = mapper.enableById(record);
    	Assert.isTrue(result == 1, "启用失败,启用成功数量为["+result+"]!");//只用当 表达式为True,才通过。
    	return 1;
    ...
    @Override
    public PageInfo<T> selectPage(T record, RowBounds rowBounds) {
    	List<T> selectList = mapper.selectByRowBounds(record, rowBounds);
