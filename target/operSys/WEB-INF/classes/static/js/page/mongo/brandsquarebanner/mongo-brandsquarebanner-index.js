layui.use(['table', 'laytpl', 'form', 'common','upload'], function() {
    ...
    form.on('submit(submit)', function(data) {
        ...
        common.wait("提交中...");
        common.post(rootPath + '/mongo/brandsquarebanner/save', data.field, function(msg) {
            common.hide();
            if (msg.ok) {
                // 提交成功刷新数据表格，关闭弹出层
    ...
    var dataTable = common.renderTable({
        url: common.getUrl('/mongo/brandsquarebanner/list'),
        ...
        switch (obj.event) {
            ...
            case 'look':
                layer.open({
                    ...
                    area: ['600px','350px'],
                    shadeClose: true,
                    content:rootPath + '/mongo/brandsquarebanner/bannerDetails?id='+data.id
                });
                break;
            case 'jump':
            ...
            case 'delete':
                layer.confirm("确定是否删除["+data.title+"]?",  {
                    ...
                    common.post( rootPath + '/mongo/brandsquarebanner/delete', {id:data.id}, function(msg) {
                        common.hide();
                        if (msg.ok) {
                            // 提交成功刷新数据表格，关闭弹出层
                            layer.alert("操作成功。",function(){
