layui.use(['table', 'laytpl', 'form', 'common','laydate','element'], function() {
    ...
    // 添加事件
		...
		form.on('submit(submit)', function(data) {
			...
			common.wait("提交中...");
			common.post(rootPath + '/wholesale/brandsquaremodular/save', data.field, function(msg) {
				common.hide();
				if (msg.ok) {
					// 提交成功刷新数据表格，关闭弹出层
    ...
    var dataTable = common.renderTable({
	    ...
	    url: common.getUrl(rootPath + '/wholesale/brandsquaremodular/list'),
	    ...
	    cols: [[
	      ...
	      {field:'columnType', title: '模块类型', width: 180,templet:function(val){
	    	  if(val.columnType=='1'){
	    		  ...
	    		  return '品牌广场热卖商品模块';
	    	  }else  if(val.columnType=='3'){
	    		  return '品牌商店模块1';
	    	  }else  if(val.columnType=='4'){
	    		  return '品牌商店模块2';
	    	  }else  if(val.columnType=='5'){
	    		  return '品牌商店模块3';
	    	  }
	      }},
          // {field:'industryName', title: '行业归属', width: 180},
		...
		case 'delete':
	   		layer.confirm("确定是否删除["+data.modularName+"]?",  {  
  			  ...
  			  ,cancel: function(index, layero){  
      			...
      			common.post( rootPath + '/wholesale/brandsquaremodular/delete', {id:data.id}, function(msg) {
      				common.hide();
      				if (msg.ok) {
      					// 提交成功刷新数据表格，关闭弹出层
      					layer.alert("操作成功。",function(){
    	...
    	if(!flag){
    		layer.confirm("确定是否禁用["+tmp.innerText+"]?",  {  
    			  ...
    			  ,cancel: function(index, layero){  
        			...
        			common.post( rootPath + '/wholesale/brandsquaremodular/updateModularStatus', {id:obj.value,modularStatus:1}, function(msg) {
        				common.hide();
        				if (msg.ok) {
        					// 提交成功刷新数据表格，关闭弹出层
        					layer.alert("操作成功。",function(){
    		...
    		layer.confirm("确定是否启用["+tmp.innerText+"]?", {  
  			...
  			//是  		common.wait("提交中...");
      			common.post( rootPath + '/wholesale/brandsquaremodular/updateModularStatus', {id:obj.value,modularStatus:2}, function(msg) {
      				common.hide();
      				if (msg.ok) {
      					// 提交成功刷新数据表格，关闭弹出层
      					layer.alert("操作成功。",function(){
