...
layui.define(['jquery', 'table','common'], function(exports) {
	"use strict";
	const $ = layui.jquery,common = layui.common,
		table = layui.table;
	...
	yutons_sug.prototype.render = function(opt) {
		...
		opt.height = opt.height || '289';
		opt.cellMinWidth = opt.cellMinWidth || '60'; //最小列宽
		//opt.page = opt.page || true;
		opt.limits = opt.limits || [50];
		opt.loading = opt.loading || true;
		opt.limit = opt.limit || 100;
		opt.size = opt.size || 'sm'; //小尺寸的表格

		//初始化输入框提示容器
		$("#" + opt.id).after("<div id='storesName' style='background-color: #fff;display: none;z-index:1;position: absolute;width:100%;'></div>");
...
/*			$("#" + opt.id).keyup(function(e) {
	...
	function getSugTable(opt) {
		...
		table.on('rowDouble(' + "yutons_sug_" + opt.id + ')', function(obj) {
			//获取选中行所传入字段的值
			$("#" + opt.id).val(obj.data.store_name);
			$("#contentUrl").val(obj.data.seq);
			$("#" + opt.id).next().hide().html("");
		});
