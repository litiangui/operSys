layui.use(['table', 'laytpl', 'form', 'common','laydate','upload'], function() {
    ...
    var dataTable = common.renderTable({
	    url: common.getUrl(rootPath + '/mongo/activity/moduleList'),
    ...
    form.on('submit(submit)', function(data) {
        ...
        common.wait("提交中...");
        common.post( rootPath + '/mongo/activity/saveModel', data.field, function(msg) {
            common.hide();
            if (msg.ok) {
                // 提交成功刷新数据表格，关闭弹出层
    	...
    	switch (obj.event) {
			case 'delete':
                layer.confirm("确认要删除吗，删除后不能恢复", { title: "删除确认" }, function (index) {
                    layer.close(index);
                    common.post(rootPath + '/mongo/activity/deleteModel', {
                        id: data.id,
                        mId:mid
                    }, function (msg) {
                        common.hide();
